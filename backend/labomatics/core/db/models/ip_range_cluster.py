from uuid import UUID

from backend.labomatics.core.db.base import Base
from backend.labomatics.core.db.mixin import TimestampMixin, UUIDPkMixin
from sqlalchemy import ForeignKey, UniqueConstraint
from sqlalchemy.orm import Mapped, mapped_column, relationship


class IpRangeCluster(Base, UUIDPkMixin, TimestampMixin):
    """Association IpRange ↔ Cluster (many-to-many)."""

    __tablename__ = "ip_range_cluster"

    ip_range_id: Mapped[UUID] = mapped_column(
        ForeignKey("ip_range.id", ondelete="RESTRICT"),
    )
    cluster_id: Mapped[UUID] = mapped_column(
        ForeignKey("cluster.id", ondelete="RESTRICT"),
    )

    # Relationships
    ip_range: Mapped["IpRange"] = relationship(
        back_populates="range_clusters",
    )
    cluster: Mapped["Cluster"] = relationship()  # type: ignore

    __table_args__ = (UniqueConstraint("ip_range_id", "cluster_id"),)


# Forward references for circular imports
from backend.labomatics.core.db.models.cluster import Cluster  # noqa: E402
from backend.labomatics.core.db.models.ip_range import IpRange  # noqa: E402
