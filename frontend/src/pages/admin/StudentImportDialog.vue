<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="uploadedFile"
        class="flex items-center gap-3 p-3 bg-surface-800 rounded"
      >
      ...
      <div
        v-if="parseError"
        class="flex items-center gap-3 p-3 bg-red-500/20 border border-red-500 rounded"
      >
    ...
    >
      ...
      <p class="text-sm text-surface-500">
        Associez les colonnes du XML aux champs de l'étudiant.
      </p>
      ...
      <div class="p-3 bg-surface-800 rounded">
        ...
        <table class="text-xs w-full">
          ...
          <tbody>
            <tr
              v-for="(row, idx) in previewRows.slice(0, 3)"
              :key="idx"
    ...
    >
      ...
      <div class="grid grid-cols-4 gap-2 text-sm mb-4">
        <Panel class="p-4 bg-green-500/20 border-2 border-green-500">
          <div class="text-white text-xs font-medium mb-2">
            Ajoutés
          </div>
        ...
        <Panel class="p-4 bg-surface-700 border-2 border-surface-600">
          ...
          <div class="text-3xl font-bold text-surface-300">
    ...
    >
      ...
      <p class="text-sm text-surface-500">
        Vérifiez les informations avant de procéder à l'import.
      </p>
      ...
      <div class="grid grid-cols-4 gap-2 text-sm">
        ...
        <Panel class="p-4 bg-surface-700 border-2 border-surface-600">
          ...
          <div class="text-3xl font-bold text-surface-300">
            {{ (diffResult?.added?.length || 0) + (diffResult?.modified?.length || 0) + (diffResult?.deleted?.length || 0) }}
          </div>
        </Panel>
      </div>
...
const previewRows = ref<ImportStudentRow[]>([])

// Étape 3
const diffResult = ref<{ added: StudentImportChange[], modified: StudentImportChange[], deleted: StudentImportChange[] } | null>(null)
...
const filteredChanges = computed(() => {
  ...
  let changes: (StudentImportChange & { status: 'added' | 'modified' | 'deleted' })[] = [
    ...diffResult.value.added.map(d => ({ ...d, status: 'added' as const })),
    ...diffResult.value.modified.map(d => ({ ...d, status: 'modified' as const })),
    ...diffResult.value.deleted.map(d => ({ ...d, status: 'deleted' as const })),
  ]
...
async function onFileSelected(event: FileSelectEvent) {
